...
int dma_example(int slot_id) {
    ...
    static const int stream_length = 100000;
    int channel = 0;

    // for each core, length of config (1) followed by one config word; then stream length and then stream
    int write_buffer_size = (NUM_CORES * 2 + 1 + stream_length) * sizeof(uint32_t);
    input = (uint32_t *)malloc(write_buffer_size);
    write_buffer = (char *)input;

    int read_buffer_size = NUM_CORES * sizeof(uint32_t);
    output = (uint32_t *)malloc(read_buffer_size);
    read_buffer = (char *)output;
    ...
    int cur_base = NUM_CORES * 2;
    input[cur_base++] = stream_length;
    ...
    uint32_t reg_peek;
    for (int i = 0; i < 1; i++) {
    // rc = fpga_pci_peek(pci_bar_handle, 0x700, &reg_peek);
    // printf("OUT: %d\n", reg_peek);
