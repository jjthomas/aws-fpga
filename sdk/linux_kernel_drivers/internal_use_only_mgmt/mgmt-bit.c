...
{
	int i;
	struct awsmgmt_ioc_bitstream bitstream_obj;
	...
	printk(KERN_INFO "%s: %s \n", DRV_NAME, __FUNCTION__);
	if (copy_from_user((void *)&bitstream_obj, arg, sizeof(struct awsmgmt_ioc_bitstream)))
		return -EFAULT;
	...
	device_info(lro, &obj);
	for (i = 0; i < 4; i++) {
		if (!obj.mig_calibration[i])
			return -ETIMEDOUT;
	}
	printk(KERN_INFO "IOCTL 6 %s:%d, err: %d\n", __FUNCTION__, __LINE__, err);
	return 0;
}

int bitstream_ioctl_axlf(struct awsmgmt_dev *lro, const void __user *arg)
{
	struct awsmgmt_ioc_bitstream_axlf bitstream_obj;
	struct axlf bin_obj;
	char __user *buffer;
	struct awsmgmt_ioc_info obj;
	long err = 0;
	...
	printk(KERN_INFO "%s: %s \n", DRV_NAME, __FUNCTION__);
	if (copy_from_user((void *)&bitstream_obj, arg, sizeof(struct awsmgmt_ioc_bitstream_axlf)))
		return -EFAULT;
