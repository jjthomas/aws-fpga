...
static void transfer_dump(struct xdma_transfer *transfer)
{
	...
	pr_info("Descriptor Entry (Pre-Transfer)\n");
...
static void engine_destroy(struct xdma_dev *xdev, struct xdma_engine *engine)
{
	...
	if (engine->desc) {
		pr_info("device %s, engine %s pre-alloc desc 0x%p,0x%llx.\n",
...
int xdma_xfer_submit(void *dev_hndl, int channel, int write, u64 ep_addr,
